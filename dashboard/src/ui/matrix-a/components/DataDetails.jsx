      ...
      {activeTab === "projects" && (
        <div className="space-y-1">
          {projectEntries.slice(0, projectLimit).map((entry) => (
            ...
            >
              <div className="w-8 h-8 rounded-md oai-bg-elevated flex items-center justify-center oai-text-caption font-medium text-oai-gray-500 dark:text-oai-gray-400">
      ...
      {activeTab === "daily" && (
        <div>
          {dailyBreakdownRows?.length === 0 ? (
            <div className="oai-text-body-sm text-oai-gray-500 dark:text-oai-gray-400 mb-4">
              {dailyEmptyPrefix}
                  ...
                  {dailyBreakdownColumns.map((column) => (
                    ...
                    >
                      <button
                        ...
                        className="flex w-full items-center justify-start px-4 py-2 text-left oai-text-caption font-semibold text-oai-gray-600 dark:text-oai-gray-400 hover:text-oai-black dark:hover:text-oai-white transition-colors"
                      >
                ...
                {dailyBreakdownRows.map((row) => (
                  <tr
                    ...
                    className={`border-b border-oai-gray-100 dark:border-oai-gray-800 last:border-b-0 hover:bg-oai-gray-50/50 dark:hover:bg-oai-gray-800/50 transition-colors ${
                      row.missing ? "text-oai-gray-400 dark:text-oai-gray-500" : row.future ? "text-oai-gray-300 dark:text-oai-gray-600" : "text-oai-black dark:text-oai-white"
                    }`}
                  >
                    <td className="px-4 py-2 oai-text-body-sm text-oai-gray-500 dark:text-oai-gray-400">
                    ...
                    <td className="px-4 py-2 oai-text-body-sm text-oai-gray-600 dark:text-oai-gray-400 tabular-nums">
          ...
          {activeTab !== "daily" && DETAILS_PAGED_PERIODS.has(period) && detailsPageCount > 1 ? (
            <div className="mt-3 flex items-center justify-between oai-text-caption">
              <button
                ...
                className="px-3 py-1.5 text-oai-gray-600 dark:text-oai-gray-400 hover:bg-oai-gray-100 dark:hover:bg-oai-gray-800 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
