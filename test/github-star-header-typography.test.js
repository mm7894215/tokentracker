...
test("GithubStar header typography matches header buttons", () => {
  const src = fs.readFileSync(
    path.join(__dirname, "../dashboard/src/ui/matrix-a/components/GithubStar.jsx"),
    "utf8",
  );
  ...
  assert.ok(headerClasses.includes("uppercase"), "expected header typography to use uppercase");
  assert.ok(headerClasses.includes("font-bold"), "expected header typography to use font-bold");
  assert.ok(
