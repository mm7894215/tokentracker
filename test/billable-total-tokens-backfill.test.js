...
test("buildCursorFilter builds composite keyset cursor", () => {
  ...
  assert.ok(
    filter.includes("and(hour_start.eq.2025-12-17T00:00:00.000Z,user_id.eq.u1,device_id.gt.d1)"),
  );
  assert.ok(
    filter.includes(
      "and(hour_start.eq.2025-12-17T00:00:00.000Z,user_id.eq.u1,device_id.eq.d1,source.gt.codex)",
    ),
  );
  assert.ok(
    filter.includes(
      "and(hour_start.eq.2025-12-17T00:00:00.000Z,user_id.eq.u1,device_id.eq.d1,source.eq.codex,model.gt.m1)",
    ),
  );
});
