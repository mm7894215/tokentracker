...
test("buildCursorFilter builds composite keyset cursor", () => {
  ...
  assert.ok(
    filter.includes(
      "and(hour_start.eq.2025-12-17T00:00:00.000Z,user_id.eq.u1,project_key.gt.victorGPT/vibeusage)",
    ),
  );
  assert.ok(
    filter.includes(
      "and(hour_start.eq.2025-12-17T00:00:00.000Z,user_id.eq.u1,project_key.eq.victorGPT/vibeusage,source.gt.codex)",
    ),
  );
});
...
test("runBackfill paginates with cursor without skipping rows", async () => {
  ...
  const uniqueKeys = new Set(
    updatesOut.map((row) => `${row.hour_start}|${row.user_id}|${row.project_key}|${row.source}`),
  );
