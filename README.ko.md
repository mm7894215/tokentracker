...
| **투명한 네트워크 활동** | [개인정보 처리방침](docs/PRIVACY.md)에 상세히 명시되어 있습니다. 기본 네트워크 요청은 프로바이더 잔여 한도 확인(기기에 이미 존재하는 자격 증명 사용), GitHub 스타 수 확인 및 버전 업데이트 확인, 익명 텔레메트리(일일 하트비트 및 PostHog 페이지뷰, 둘 다 `TOKENTRACKER_NO_TELEMETRY=1` 또는 `DO_NOT_TRACK=1`로 비활성화 가능)로 제한됩니다. 텔레메트리에는 토큰, 모델, 프롬프트, 코드가 절대 포함되지 않습니다. |
