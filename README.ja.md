...
| **明確なネットワーク通信** | [プライバシーポリシー](docs/PRIVACY.md)にすべて記載されています。デフォルトのリクエストは、プロバイダーの残量確認（マシン上の既存の認証情報を使用）、GitHub スター数取得やアップデート確認、匿名のテレメトリ（毎日のハートビートと PostHog ページビュー。`TOKENTRACKER_NO_TELEMETRY=1` または `DO_NOT_TRACK=1` で無効化可能）に限られます。テレメトリにトークン、モデル、プロンプト、コードが含まれることはありません。 |
