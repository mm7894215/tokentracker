...
- 🔒 **プライバシー最優先** — トークン数とタイムスタンプのみを記録。プロンプト、生成テキスト、コードがマシン外に出ることはありません。
...
| 機能 | **[TokenTracker](https://github.com/xiufengsun/TokenTracker)** | **[ccusage](https://github.com/ryoppippi/ccusage)** | **[Tokscale](https://github.com/junhoyeo/tokscale)** |
|---|:---:|:---:|:---:|
| **対応 AI ツール数** | **36** | マルチエージェント | マルチエージェント |
...
| **明確なネットワーク通信** | [プライバシーポリシー](docs/PRIVACY.md)にすべて記載されています。デフォルトのリクエストは、プロバイダーの残量確認（マシン上の既存の認証情報を使用）、アップデート確認、匿名のテレメトリ（毎日のハートビートと PostHog ページビュー。`TOKENTRACKER_NO_TELEMETRY=1` または `DO_NOT_TRACK=1` で無効化可能）に限られます。テレメトリにトークン、モデル、プロンプト、コードが含まれることはありません。 |
