...
Usage rollup 目前为临时降级状态，后端强制走 hourly 聚合（避免在缺表环境触发 `vibeusage_tracker_daily_rollup` 查询错误）。当前版本忽略 `VIBEUSAGE_ROLLUP_ENABLED`。
...
1. 在 InsForge SQL 控制台执行：
   ...
   - `scripts/ops/usage-daily-rollup-backfill.sql`（随后调用 `vibeusage_rebuild_daily_rollup` 回填）
